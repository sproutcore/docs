...
function publish(symbolSet) {
  ...
  for (var i = 0, l = symbols.length; i < l; i++) {
    ...
    var splitSrcFilePath = srcFilePath.split( "/" );
    var fileName = splitSrcFilePath ? splitSrcFilePath[ splitSrcFilePath.length - 1 ] : null;
    var useSrcPath = fileName;

    useSrcPath = useSrcPath.replace(/\:/g, "_");
    symbol.srcFile = useSrcPath;
  }
...
function  saveDashIndex( classes, conf )
{
  ...
  for( var i = 0; i < classes.length; ++i )
  {
    ...
    if( !isGlobal )
      classScript += insertToSearchIndex( cName, "cl", classPath ) + "\n";
    ...
    for( var m = 0; m < ownProperties.length; ++m )
    {
      ...
      var type = isGlobal ? "Global" : "instp";
      var path = classPath + "#" + symbol.name;
