...
/** @scope Docs.classController.prototype */ {
  ...
  _scrollToEntity: function(entity) {
    var container = $('#class'),
        entityElem = entity ? container.find('div[name="' + entity + '"]') : null,
        top = entityElem && entityElem[0] ? this._positionInContainer(entityElem[0], container[0]).top - 15 : 0;

    container.stop();
    container.animate({scrollTop: top}, 400, function() {
      entityElem.find('.header').glow();
    });
  },
